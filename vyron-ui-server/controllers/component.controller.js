...
const hasMatchingNamedExport = (code, name) => {
  const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`export\\s+(const|function|class)\\s+${escaped}\\b`).test(code);
};
...
export const saveComponent = async (req, res) => {
  try {
    ...
    if (!isValidComponentName(componentName)) {
      return res.status(400).json({
        message: "Component name must be PascalCase without spaces, e.g. ProductCard"
      });
    }

    if (!hasMatchingNamedExport(code, componentName)) {
      return res.status(400).json({
        message: `Code must include a named export matching the component name: export const ${componentName} = ...`
      });
    }
    ...
    if (user.role === "admin") {
      const existing = await Component.findOne({ name: componentName, visibility: "public" });
...
export const publishComponent = async (req, res) => {
  try {
    ...
    if (!isValidComponentName(component.name)) {
      return res.status(400).json({
        message: "Component name must be PascalCase without spaces before publishing"
      });
    }

    if (!hasMatchingNamedExport(component.code, component.name)) {
      return res.status(400).json({
        message: `Code must include a named export matching the component name: export const ${component.name} = ...`
      });
    }
    ...
    const libPath = path.join(process.cwd(), "../vyron-ui-lib");
    ...
    component.npmPackage = "vyron-ui-library-ai";
    component.npmPublishStatus = "not_published";
    component.npmPublishError = undefined;

    await component.save();

    let npmPublished = false;
    let npmPublishError = null;

    try {
      console.log("Publishing to npm...");
      // Using --access public to ensure it works for scoped packages if any
      execSync("npm publish --access public", {
        cwd: libPath,
        stdio: "inherit"
      });

      component.npmPublishStatus = "published";
      npmPublished = true;
    } catch (publishError) {
      npmPublishError = publishError.message;
      
      // Check for 2FA error specifically to guide the user
      if (npmPublishError.includes("Two-factor authentication") || npmPublishError.includes("403 Forbidden")) {
         npmPublishError = "NPM Publish failed: 2FA is required. Please use an NPM Automation Token in your .npmrc to allow automated publishing.";
      }
      
      component.npmPublishStatus = "failed";
      component.npmPublishError = npmPublishError;
    }

    await component.save();

    const publishedComponent = await Component.findById(component._id)
      .populate("owner", "name email");

    res.json({
      message: npmPublished
