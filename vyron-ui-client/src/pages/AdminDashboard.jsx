...
import { setAllComponents, setUserData } from "../redux/userSlice";
...
function AddComponentForm({ onSaved }) {
  ...
  const handleSave = async () => {
    ...
    if (!/^[A-Z][A-Za-z0-9]*$/.test(name.trim())) {
      showToast("Use PascalCase without spaces, e.g. ProductCard.", "error");
      return;
    }
    setSaving(true);
    try {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setIsPublished(true);
      showToast(
        res.data?.npmPublished
          ? "Published to npm successfully!"
          : "Component is public, but npm publish failed. Check server logs.",
        res.data?.npmPublished ? "success" : "info"
      );
      onSaved && onSaved();
    } catch (err) {
            ...
            {codeTab === "code" ? (
              <motion.div key="code-editor" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <textarea
                  ...
                  rows={12}
                  spellCheck={false}
                  placeholder={`export const ProductCard = ({ title = "Product" }) => {\n  return (\n    <div>\n      <h1>{title}</h1>\n    </div>\n  );\n};`}
...
export default function AdminDashboard() {
  ...
  const refreshComponents = async () => {
    try {
      const res = await axios.get(`${ServerUrl}/api/component/all-components`, {
        withCredentials: true,
      });
  ...
  if (!userData || userData.role !== "admin") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#030b0d] text-white">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeView === "add" && (
            <motion.div
              ...
              transition={{ duration: 0.25 }}
            >
              <AddComponentForm onSaved={refreshComponents} />
            </motion.div>
          )}
        </AnimatePresence>
      </main>
