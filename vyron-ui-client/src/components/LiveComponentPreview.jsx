...
export default function LiveComponentPreview({ code }) {
  ...
  sanitized = sanitized
    .replace(/ReactDOM\.render\s*\([\s\S]*?\);?/g, "")
    .replace(/createRoot\s*\([\s\S]*?\)\.render\s*\([\s\S]*?\);?/g, "")
    .replace(/ReactDOM\.createRoot\s*\([\s\S]*?\)\.render\s*\([\s\S]*?\);?/g, "")
    .replace(/const\s+\w+\s*=\s*ReactDOM\.createRoot\s*\([\s\S]*?\);\s*\w+\.render\s*\([\s\S]*?\);?/g, "")
    .replace(/const\s+\w+\s*=\s*createRoot\s*\([\s\S]*?\);\s*\w+\.render\s*\([\s\S]*?\);?/g, "")
  ...
  return (
    ...
    >
      ...
      <LiveProvider
        key={refreshKey}
        code={wrappedCode}
        scope={{ React, ReactDOM, createRoot, useState, useEffect, useRef, useCallback }}
        noInline
      >
        {/* 🔥 MAIN CONTAINER */}
        <motion.div
